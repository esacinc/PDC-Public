...
import { Component, Renderer, ElementRef, AfterViewChecked } from '@angular/core';
...
export class AppComponent implements AfterViewChecked {
  title = 'app';
  constructor(private router: Router,  private overlayWindow: OverlayWindowService, private elRef:ElementRef, private _renderer: Renderer) {
  ...
  ngAfterViewChecked() {
      ...
      for (var i = 0; i < hotjarButtonElements.length; i++) {
        this._renderer.setElementAttribute(hotjarButtonElements[i], 'aria-label', 'Hotjar Feedback button');
      }
      ...
      for (var i = 0; i < hotjarInputElements.length; i++) {
        this._renderer.setElementAttribute(hotjarInputElements[i], 'title', 'Hotjar Input button');
      }
      ...
      for (var i = 0; i < matExpansionIndicator.length; i++) {
        this._renderer.setElementAttribute(matExpansionIndicator[i], 'title', 'Expand/Collapse the charts section');
      }    
  }
}
