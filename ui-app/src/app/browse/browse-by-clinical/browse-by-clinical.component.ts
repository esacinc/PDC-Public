...
@Component({
  ...
  styleUrls: ['../../../assets/css/global.css', './browse-by-clinical.component.css', '../browse-table.component.css'],
...
export class BrowseByClinicalComponent implements OnInit {
  ...
  /*["project_name","primary_site","program_name","disease_type","analytical_fraction","experiment_type","acquisition_type","study_name","submitter_id_name","sample_type","ethnicity","race","gender","tumor_grade","data_category","file_type","access","downloadable","studyName_genes_tab", "case_status", "biospecimen_status"];*/
...
//@@@PDC-3667: "Select all pages" option issue
  ...
  populateAssociatedSampleInfo(filteredClinicalData) {
    if (filteredClinicalData.length > 0) {
      ...
      this.filteredClinicalData = updatedClinicalData;
    }
  ...
  private trackCurrentPageSelectedCase(filteredFilesData: AllClinicalData[]) {
    ...
    this.selectedClinicalData.forEach(item => {
      if (fileIdList.indexOf(item.case_submitter_id) !== -1) {
        this.currentPageSelectedClinical.push(item.case_submitter_id);
      }
    });
  }
