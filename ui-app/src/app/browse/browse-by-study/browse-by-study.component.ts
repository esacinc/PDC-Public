...
export class BrowseByStudyComponent implements OnInit, OnChanges {
  ...
  /*["project_name","primary_site","program_name","disease_type","analytical_fraction","experiment_type","acquisition_type","study_name","submitter_id_name","sample_type","ethnicity","race","gender","tumor_grade","data_category","file_type","access","downloadable","studyName_genes_tab", "biospecimen_status", "case_status"];*/
  ...
  // Dropdown state for export buttons
  showExportAllDropdown: boolean = false;
  showExportStudyDropdown: boolean = false;
      ...
      this.browseByStudyService.getFilteredStudiesPaginated(0, 0, this.sort, this.newFilterSelected, true).subscribe((data: any) =>{
        ...
        if (buttonClick) {
          ...
          for(let study of data.getPaginatedUIStudy.uiStudies){
            //console.log("Click Full study: "+ JSON.stringify(study));
  ...
  // Dropdown toggle methods for export buttons
  toggleExportAllDropdown() {
    this.showExportAllDropdown = !this.showExportAllDropdown;
    // Close the other dropdown if it's open
    if (this.showExportAllDropdown) {
      this.showExportStudyDropdown = false;
    }
  }

  toggleExportStudyDropdown() {
    this.showExportStudyDropdown = !this.showExportStudyDropdown;
    // Close the other dropdown if it's open
    if (this.showExportStudyDropdown) {
      this.showExportAllDropdown = false;
    }
  }
}
