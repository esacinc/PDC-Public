...
import {
  AllStudiesData, Filter, WorkflowMetadata, ProtocolData, PublicationData,
...
export class StudySummaryComponent implements OnInit {
  ...
  //@@@PDC-8940 - multiple protocols for a study
  ...
  has_lipidome_data = false;
  has_metabolome_data = false;

  // Dropdown state variables
  showLipidomeDropdown = false;
  showMetabolomeDropdown = false;

  selectedTabIndex = TAB_INDEXES.DESCRIPTION;
  //@@@PDC-8940 - UI support for multiple protocols
  isTableVisible: boolean[] = [];
  protocol_length: number;
  ...
  constructor(private route: ActivatedRoute, private router: Router, private apollo: Apollo, private http: HttpClient,
    ...
    /*{"program_name" : "", "project_name": "", "study_name": "", "submitter_id_name": "", "disease_type":"", "primary_site":"", "analytical_fraction":"", "experiment_type":"",
    ...
    //@@@PDC-9142 - TMT18 not display channel aliquot
    ...
    this.studyExperimentDesignMap = [{
      ...
      {
        'id': "TMT18",
        'cols': "tmt_126,tmt_127n,tmt_127c,tmt_128n,tmt_128c,tmt_129n,tmt_129c,tmt_130n,tmt_130c,tmt_131,tmt_131c,tmt_132n,tmt_132c,tmt_133n,tmt_133c,tmt_134n,tmt_134c,tmt_135n"
      }, {
        'id': "N/A",
        'cols': ""
      }];
  ...
  showVersionStudySummary(version: string) {
    ...
    this.loading = true;
    this.studySummaryService.getFilteredStudyData(this.study_submitter_id_name, '', version, this.source).subscribe((data: any) => {
      ...
      this.has_metabolome_data = this.studySummaryData.has_metabolome_data === 'Yes';
      ...
      // Refresh file counts and other version-specific data
      this.getFilesCountsPerStudy();
      this.getUnfilteredCaseCount(this.pdcStudyID, this.studyVersion);

      this.loading = false;

      //@@@PDC-2912 - show "N/A" instead of empty embargo date
  ...
  getStudySummaryData() {
    ...
    setTimeout(() => {
      this.studySummaryService.getFilteredStudyData(this.study_submitter_id_name, '', '', this.source).subscribe((data: any) => {
        ...
        //Only set to most recent version if user hasn't manually selected a specific version
        if (!this.manualVersionSelected &&
    ...
    setTimeout(() => {
      this.studySummaryService.getProtocolData(this.study_id, this.source).subscribe((data: any) => {
        ...
        if (Array.isArray(this.protocol_data)) {
          ...
          this.isTableVisible = new Array(this.protocol_data.length).fill(false);
        } else {
          console.error('protocol_data is not an array');
        }
        this.loading = false;
      });
    }, 1000);
  ...
  //@@@PDC-8940-UI-support-multiple-protocols
  toggleProtocolTableVisibility(index: number): void {
  ...
  getFilesCountsPerStudy() {
    ...
    //@@@PDC-10273-data-category-order-study-summary
    if (this.fileCountsRaw != undefined) {
      ...
      this.studySummaryService.getFilteredClinicalDataPaginated(this.offset, this.limit, this.sort, this.newFilterSelected, this.source).subscribe((data: any) => {
        ...
        setTimeout(() => {
          this.studySummaryService.getFilteredClinicalDataPaginatedPost(this.offset, this.totalRecordsClinical, this.sort, this.newFilterSelected, this.source).pipe(take(1)).subscribe((data: any) => {
            this.filteredClinicalData = _.cloneDeep(data.getPaginatedUIClinical.uiClinical);
...
//PDC-2998 - update UI to include API changes for study versions new feature
  ...
  // Methods for handling multiomics dropdowns
  toggleStudiesDropdown(type: string, event: Event) {
    event.stopPropagation();

    if (type === 'lipidome') {
      this.showLipidomeDropdown = !this.showLipidomeDropdown;
      this.showMetabolomeDropdown = false; // Close other dropdown
    } else if (type === 'metabolome') {
      this.showMetabolomeDropdown = !this.showMetabolomeDropdown;
      this.showLipidomeDropdown = false; // Close other dropdown
    }

    // Add click listener to close dropdown when clicking outside
    if (this.showLipidomeDropdown || this.showMetabolomeDropdown) {
      document.addEventListener('click', this.closeDropdowns.bind(this));
    }
  }

  closeDropdowns() {
    this.showLipidomeDropdown = false;
    this.showMetabolomeDropdown = false;
    document.removeEventListener('click', this.closeDropdowns.bind(this));
  }

  getLipidomeStudies() {
    return this.entityReferenceInternalData.filter((study: any) =>
      study.submitter_id_name && (
        study.submitter_id_name.toLowerCase().includes('lipidome') ||
        study.submitter_id_name.toLowerCase().includes('lipidomics')
      )
    );
  }

  getMetabolomeStudies() {
    return this.entityReferenceInternalData.filter((study: any) =>
      study.submitter_id_name && (
        study.submitter_id_name.toLowerCase().includes('metabolome') ||
        study.submitter_id_name.toLowerCase().includes('metabolomics')
      )
    );
  }
...
//@@@PDC-3928 when downloading files for specific study version have to provide study UUID
  showFilesOverlay(submitter_id_name, data_category_val, file_type_val) {
    ...
    dialogConfig.data = {
      summaryData: {
        study_name: submitter_id_name,
        study_id: this.study_id,
        data_category: data_category_val,
        file_type: file_type_val,
        versions: versioned_study,
        acquisition_type: "",
        experiment_type: "",
        currentVersion: this.studyVersion
      },
    };
  ...
  close(navigateToHeatmap: boolean, study_name: string = '') {
    ...
    if (navigateToHeatmap) {
      ...
      this.router.navigate([]).then(result => {
        var url = "/pdc/analysis/" + this.study_id + "?StudyName=" + study_name;
...
//Will not close study summary upon opening a HeatMap in a separate tab
  ...
  ngOnInit() {
    ...
    //@@@PDC-8986 add protocol to experiment design table
    this.studyExperimentDesignTableFrozenCols = this.studyExperimentDesignTableCommonCols = [
      ...
      {field: 'protocol_submitter_id', header: 'Protocol'}
    ...
    this.clinicalExportCols = [
      ...
      {field: 'colon_polyps_history', header: 'Colon Polyps History'},
      {field: 'days_to_best_overall_response', header: 'Days to Best Overall Response'},
...
//helper function preparing a string containing the data for TSV manifest file
  ...
  prepareTreatmentDataforExport(exportData, format) {
    ...
    for (var i = 0; i < this.treatmentCols.length; i++) {
      ...
      headerCols.push(this.treatmentCols[i]['header']);
    }
    let exportFileTreatmentObject = JSON.parse(JSON.stringify(treatmentDataArr, treatmentColValues));
    if (format == "tsv") {
      let exportTreatmentTSVData = this.prepareTSVExportManifestData(exportFileTreatmentObject, headerCols);
      treatmentBlob = new Blob([exportTreatmentTSVData], {type: 'text/csv;charset=utf-8;'});
    } else if (format == "csv") {
      let exportTreatmentCSVData = this.prepareCSVExportManifestData(exportFileTreatmentObject, headerCols);
...
//@@@PDC-4490: Update Clinical manifest and Case summary pages for GDC Sync
  ...
  prepareDownloadData(format, exportData, exportFileObject) {
    ...
    //@@@PDC-8452 name the archive correctly
    let folderName = this.getExportFileName(format, "", "true").replace("." + format, "") + ".zip";
    //Download files
...
//@@@PDC-1219: Add a new experimental design tab on the study summary page
  getStudyExperimentalDesign() {
    ...
    setTimeout(() => {
      ...
      this.studySummaryService.getBiospecimenPerStudy(this.study_id, this.source).pipe(take(1)).subscribe((biospecimenData: any) =>{
        this.biospecimenPerStudy = _.cloneDeep(biospecimenData.uiBiospecimenPerStudy);
        ...
        setTimeout(() => {
          this.studySummaryService.getStudyExperimentalDesign(this.study_id, this.source).pipe(take(1)).subscribe((data: any) => {
            this.studyExperimentalDesign = _.cloneDeep(data.uiStudyExperimentalDesign);
            ...
            for (let studyExpData of this.studyExperimentalDesign) {
              experimentType = studyExpData["experiment_type"];
            }
            let colsSpecificToExpType = [];
  ...
  async downloadStudySummary() {
    await this.studySummaryService.downloadStudyWorkbook(this.study_id);
  }
  ...
  protected readonly TAB_INDEXES = TAB_INDEXES;
}
