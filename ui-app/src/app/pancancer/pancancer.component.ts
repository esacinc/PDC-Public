...
import {
  publicationsStudyData,
  PublicationsData,
  QueryPublicationsData,
  publicationsFiltersData,
  UIPublicationsData
} from '../types';
    ...
    setTimeout(() => {
      this.publicationsService.getPublicationsResults().pipe(take(1)).subscribe((data: any) => {
        if (data && data.getUIPancancerPublications) {
          this.allPublicationsData = _.cloneDeep(data.getUIPancancerPublications);
          //@@@PDC-7267 - update order of publications so that publication_id "eae0a229-3ade-11ee-b8fc-0aaebe6fe27b" is always first
          //@@@PDC-10425 - updates to restore publication order
          for (let data of this.allPublicationsData) {
            if (data.title == "Proteogenomic data and resources for pan-cancer analysis") {
              this.allPublicationsData.splice(this.allPublicationsData.indexOf(data));
              this.allPublicationsData.unshift(data);
            }
          }
          //@@@PDC-9746 - update order of publications
          // Keep the first record in place and sort the rest by year in descending order
          const firstPublication = this.allPublicationsData[0];
          const sortedPublications = this.allPublicationsData.slice(1);
          sortedPublications.sort((a, b) => Number(b.year) - Number(a.year));
          this.allPublicationsData = [firstPublication, ...sortedPublications];
