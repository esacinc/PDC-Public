...
const createHumanBody: TCreateHumanBody = ({
 ...
 // var viewBoxWidth = width + 150;
  ...
  var viewBoxHeight = height + 550; // was +50
var viewBoxWidth = width + 300;   // was +150
  // The Bar Chart
  ...
  const svg = d3
    ...
    .attr('width', '1200')
    .attr('height', '500')
    //.attr('viewBox', `0 250 1700 500`)
    .attr('viewBox', `-320 350 2000 600`)
	.attr('style', 'position:relative;left:200px;margin:0px;padding:0px;')
    .append('g');
  ...
  xAxisLabels.append("text")
   ...
   .attr('y', (d, i) => ((plotHeight / (numberofOrgans-1)) * i) + 6 + (50 / 2))
    ...
    if (mouseOverHandler) {
      ...
      var tooltipLabelPostfix = d['_count'] > 1 ? 'cases': 'case';
       tooltip
         ...
         .style('left', `${i['clientX'] - root.getBoundingClientRect().left + 20}px`)
         .style('top', `${i['clientY'] - root.getBoundingClientRect().top}px`)
