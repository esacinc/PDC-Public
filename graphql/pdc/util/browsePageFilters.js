...
//@@@PDC-10091 exclude Aebersold Lab
...
//@@@PDC-9979 API change for new filters
const al_sam_ca_dem_dia_filter = `
    SELECT DISTINCT s.study_submitter_id,
                    s.pdc_study_id,
                    s.submitter_id_name,
                    s.acquisition_type,
                    s.analytical_fraction,
...
//@@@PDC-9845 use program short name in filter
//@@@PDC-9903 hide Quantitative Digital Maps program
//@@@PDC-9921 add new filters
const prog_proj_al_sam_ca_dem_dia_filter = `
    SELECT DISTINCT s.study_submitter_id,
                    ...
                    s.analytical_fraction,
                    s.experiment_type,
                    prog.program_shortname AS program_name,
                    proj.name              AS project_name,
                    al.status              AS biospecimen_status,
                    sam.sample_type,
                    c.disease_type,
                    ...
                    treat.therapeutic_agents,
                    treat.treatment_intent_type,
                    treat.treatment_outcome,
                    treat.treatment_type,
                    expo.alcohol_history,
                    expo.alcohol_intensity,
                    expo.tobacco_smoking_status,
                    expo.cigarettes_per_day
    FROM study s
             JOIN project proj ON proj.project_id = s.project_id
             JOIN program prog ON proj.program_id = prog.program_id
             JOIN aliquot_run_metadata alm ON alm.study_id = s.study_id
...
//@@@PDC-9620 - fix file overlay display multiple protocol
const file_tab_data=`
SELECT DISTINCT
    ...
    srm.protocol_submitter_id,
    srm.protocol_submitter_id as protocol,
    proj.name AS project_name,
    f.data_category,
    f.file_type,
    f.downloadable,
...
JOIN study_file sf on s.study_id = sf.study_id
JOIN file f on sf.file_id = f.file_id
JOIN project proj on proj.project_id = s.project_id
JOIN program prog on proj.program_id = prog.program_id
LEFT JOIN study_run_metadata srm 
    ON sf.study_run_metadata_submitter_id = srm.study_run_metadata_submitter_id
        AND s.study_id = srm.study_id

WHERE
    'PG25730263' = 'PG25730263'
        
`;

const case_tab_count = `
    SELECT COUNT(DISTINCT al.aliquot_id) AS total
    FROM study s
             JOIN project proj ON proj.project_id = s.project_id
             JOIN program prog ON proj.program_id = prog.program_id
             JOIN aliquot_run_metadata alm ON alm.study_id = s.study_id
...
const case_tab_data = `
    SELECT DISTINCT proj.name                  AS project_name,
                    prog.name                  AS program_name,
...
const clinical_tab_count = `
    SELECT COUNT(DISTINCT dia.diagnosis_id) AS total
    FROM study s
             JOIN project proj ON proj.project_id = s.project_id
             JOIN program prog ON proj.program_id = prog.program_id
             JOIN aliquot_run_metadata alm ON alm.study_id = s.study_id
...
const clinical_tab_data = `
    SELECT DISTINCT prog.name                                                 AS program_name,
                    proj.name                                                 AS project_name,
                    BIN_TO_UUID(c.case_id)                                    AS case_id,
                    c.case_submitter_id,
                    c.status,
                    ...
                    IFNULL(treat.therapeutic_agents, ' ')                     as therapeutic_agents,
                    IFNULL(treat.treatment_intent_type, ' ')                  as treatment_intent_type,
                    IFNULL(treat.treatment_outcome, ' ')                      as treatment_outcome,
                    IFNULL(treat.treatment_type, ' ')                         as treatment_type,
                    IFNULL(expo.alcohol_history, ' ')                         as alcohol_history,
                    IFNULL(expo.alcohol_intensity, ' ')                       as alcohol_intensity,
                    IFNULL(expo.tobacco_smoking_status, ' ')                  as tobacco_smoking_status,
                    IFNULL(expo.cigarettes_per_day, ' ')                      as cigarettes_per_day
    FROM study s
             JOIN project proj ON proj.project_id = s.project_id
             JOIN program prog ON proj.program_id = prog.program_id
             JOIN aliquot_run_metadata alm ON alm.study_id = s.study_id
...
const study_tab_count = `
    SELECT COUNT(DISTINCT s.study_id) AS total
    FROM study s
             JOIN project proj ON proj.project_id = s.project_id
             JOIN program prog ON proj.program_id = prog.program_id
             JOIN aliquot_run_metadata alm ON alm.study_id = s.study_id
...
const study_tab_data = `
    SELECT BIN_TO_UUID(s.study_id)             AS study_id,
           s.study_submitter_id,
           s.pdc_study_id,
           s.submitter_id_name,
           s.study_description,
...
//@@@PDC-9845 use program short name
const prog_proj_filter_columns = {
  program_name: "prog.program_shortname",
...
const al_sam_ca_dem_dia_filter_column = {
  //biospecimen_status: "al.status",
	...
	vital_status: "dem.vital_status",
	age_at_diagnosis: "dia.age_at_diagnosis",
	ajcc_clinical_stage: "dia.ajcc_clinical_stage",
	ajcc_pathologic_stage: "dia.ajcc_pathologic_stage",
	progression_or_recurrence: "dia.progression_or_recurrence",
	therapeutic_agents: "treat.therapeutic_agents",
	treatment_intent_type: "treat.treatment_intent_type",
	treatment_outcome: "treat.treatment_outcome",
	treatment_type: "treat.treatment_type",
	alcohol_history: "expo.alcohol_history",
	alcohol_intensity: "expo.alcohol_intensity",
	tobacco_smoking_status: "expo.tobacco_smoking_status",
	cigarettes_per_day: "expo.cigarettes_per_day"
};
...
const fileFilterCategory = ["data_category"];
